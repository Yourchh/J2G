...
public class ValidadorEstructural {
    ...
    private Map<String, String> cargarReglasRegexEnMemoria() {
        ...
        String anyLiteralOrVarRegex = "(" + varOrIdPatternCore + "|" + stringLiteralRegex + "|" + numberLiteralRegex + "|" + booleanLiteralRegex + ")";
        ...
        reglas.put("INPUT_STR_STMT", "^(?:" + varOrIdPatternCore + "\\s*:=\\s*)?Input\\s*" + optionalPromptRegex + "\\.Str\\(\\)\\s*;");
        reglas.put("INPUT_INT_STMT", "^(?:" + varOrIdPatternCore + "\\s*:=\\s*)?Input\\s*" + optionalPromptRegex + "\\.Int\\(\\)\\s*;");
        reglas.put("INPUT_BOOL_STMT", "^(?:" + varOrIdPatternCore + "\\s*:=\\s*)?Input\\s*" + optionalPromptRegex + "\\.Bool\\(\\)\\s*;");
        ...
        reglas.put("FOR_STMT", "^\\s*for\\s*\\(\\s*(INT\\s+[a-z][a-zA-Z0-9_]*\\s*:=\\s*[0-9]+|[a-z][a-zA-Z0-9_]*\\s*:=\\s*[0-9]+)\\s*:\\s*([^:]+)\\s*:\\s*([^)]+)\\s*\\)\\s*\\{\\s*$"); 
            ...
            if (symbolPairs.containsValue(c)) { 
                ...
                if (stack.isEmpty()) {
                    ...
                    errors.add("Error de sintaxis: Símbolo de cierre '" + c + "' en columna " + (i + 1) + 
                               " no coincide con el símbolo de apertura esperado '" + getMatchingClosingSymbol(stack.peek()) + "' (abierto en columna " + (positionStack.peek()+1) + ") " + contextDesc + ".");
                } else {
                    stack.pop();
                    positionStack.pop(); 
                }
    ...
    private String getExpressionType(String expression, Map<String, String> reglas, Map<String, String> declaredVariablesTypeMap) {
        if (expression.matches(reglas.get("STRING_LITERAL"))) return "STR";
        if (expression.matches(reglas.get("NUMBER_LITERAL"))) return "INT";
        if (expression.matches(reglas.get("BOOLEAN_LITERAL"))) return "BOOL";
        if (declaredVariablesTypeMap.containsKey(expression)) return declaredVariablesTypeMap.get(expression);
        if (expression.startsWith("Input.") && expression.endsWith("()")) {
            if (expression.contains(".Str()")) return "STR";
            if (expression.contains(".Int()")) return "INT";
            if (expression.contains(".Bool()")) return "BOOL";
        }
        return "UNKNOWN"; 
    }
            ...
            if (op.equals(":=")) { 
                ...
                if (!lhsType.equals("INT")) {
                    errorsEnLinea.add("Error de tipo: Operador de asignación compuesta '" + op + 
                                      "' requiere que la variable '" + lhsVar + "' sea de tipo INT, pero es " + lhsType + " " + context + ".");
                }
                if (!rhsType.equals("UNKNOWN") && !rhsType.equals("INT")) { 
                     errorsEnLinea.add("Error de tipo: Operador de asignación compuesta '" + op + 
                                      "' requiere que el valor a la derecha sea de tipo INT, pero es " + rhsType + " " + context + ".");
                }
            }
        }
    }
    ...
    public void validarEstructuraConRegex(String codigoLimpioFormateado) {
        ...
        for (int i = 0; i < lineasCodigo.length; i++) {
            ...
            for (char ch : originalLineForBraceCheck.toCharArray()) {
                if (ch == '{') { globalBraceBalance++; lineBraceBalance++;}
                else if (ch == '}') { globalBraceBalance--; lineBraceBalance--;}
            }
            if (globalBraceBalance < 0 && lineBraceBalance < 0 && errorsEnLinea.stream().noneMatch(err -> err.contains("inesperada"))) {
            ...
            if (lineaActual.matches(reglas.getOrDefault("MAIN_FUNC_START", "^$"))) {
                ...
                if (lineaActual.matches(reglas.getOrDefault("SWITCH_STMT", "^$"))) {
                    ...
                    if(switchMatcher.matches()){
                        ...
                        if (this.currentSwitchExpressionType.equals("UNKNOWN") && switchExpressionText.matches(varOrIdCorePattern) && !declaredVariablesTypeMap.containsKey(switchExpressionText)) {
                     ...
                     if (lineaActual.matches(reglas.getOrDefault("CASE_STMT", "^$")) || lineaActual.matches(reglas.getOrDefault("DEFAULT_STMT", "^$"))) {
                        ...
                        if(lineaActual.matches(reglas.getOrDefault("CASE_STMT", "^$"))) {
                             ...
                             if(caseMatcher.matches()){
                                ...
                                errorsEnLinea.addAll(checkExpressionVariables(actualCaseValue, "en valor de caso '" + actualCaseValue + "'", reglas, declaredVariablesTypeMap, currentLineNumber));
                                String caseValueType = getExpressionType(actualCaseValue, reglas, declaredVariablesTypeMap);

                                if (this.currentSwitchExpressionType != null && !this.currentSwitchExpressionType.equals("UNKNOWN") &&
                                    !caseValueType.equals("UNKNOWN") && !this.currentSwitchExpressionType.equals(caseValueType)) {
                                ...
                                } else if (caseValueType.equals("UNKNOWN") && !actualCaseValue.matches(varOrIdCorePattern) && !actualCaseValue.isEmpty()){
                ...
                if (!reglaCoincidioEstaLinea) {
                    ...
                    if (matcher.matches()) {
                        ...
                        if (!initPart.isEmpty()) {
                            ...
                            Matcher initDeclMatcher = Pattern.compile("^(INT|STR|BOOL)\\s+" + varOrIdCorePattern + "\\s*:=\\s*(.+)$").matcher(initPart);
                            Matcher initAssignMatcher = Pattern.compile("^" + varOrIdCorePattern + "\\s*(:=|\\+=|-=|\\*=|\\/=)\\s*(.+)$").matcher(initPart);
                            ...
                            if (initDeclMatcher.matches()) {
                                ...
                                errorsEnLinea.addAll(checkVariableUsage(varName, contextFor + " (declaración)", reglas, declaredVariablesTypeMap));
                                errorsEnLinea.addAll(checkExpressionVariables(rhsAssigned, contextFor + " (RHS para " + varName + ")", reglas, declaredVariablesTypeMap, currentLineNumber));
                                performAssignmentTypeChecks(varName, ":=", rhsAssigned, contextFor, reglas, declaredVariablesTypeMap, errorsEnLinea);
                                ...
                                errorsEnLinea.addAll(checkVariableUsage(lhsVar, contextFor + " (LHS asignación)", reglas, declaredVariablesTypeMap));
                                errorsEnLinea.addAll(checkExpressionVariables(rhsEx, contextFor + " (RHS asignación para " + lhsVar + ")", reglas, declaredVariablesTypeMap, currentLineNumber));
                                performAssignmentTypeChecks(lhsVar, op, rhsEx, contextFor, reglas, declaredVariablesTypeMap, errorsEnLinea);
                            } else {
                                errorsEnLinea.add("Error de sintaxis: Parte de inicialización del for ('" + initPart + "') no es una declaración (ej: INT i := 0) o una asignación válida.");
                                errorsEnLinea.addAll(checkExpressionVariables(initPart, contextFor, reglas, declaredVariablesTypeMap, currentLineNumber));
                            }
                        }
                        ...
                        if (!updatePart.isEmpty()) {
                            Matcher updateAssignMatcher = Pattern.compile("^" + varOrIdCorePattern + "\\s*(:=|\\+=|-=|\\*=|\\/=)\\s*(.+)$").matcher(updatePart);
                             if (updateAssignMatcher.matches()) {
                                ...
                                errorsEnLinea.addAll(checkVariableUsage(lhsVar, contextFor + " (LHS actualización)", reglas, declaredVariablesTypeMap));
                                errorsEnLinea.addAll(checkExpressionVariables(rhsEx, contextFor + " (RHS actualización para " + lhsVar + ")", reglas, declaredVariablesTypeMap, currentLineNumber));
                                performAssignmentTypeChecks(lhsVar, op, rhsEx, contextFor, reglas, declaredVariablesTypeMap, errorsEnLinea);
                            } else {
                                errorsEnLinea.add("Error de sintaxis: Parte de actualización del for ('" + updatePart + "') no es una asignación válida (ej: i += 1).");
                                errorsEnLinea.addAll(checkExpressionVariables(updatePart, contextFor, reglas, declaredVariablesTypeMap, currentLineNumber));
                            }
                        }
                    }
                }
                ...
                if (lineaActual.matches(reglas.getOrDefault("BLOCK_END", "^$"))) { 
                    if (!reglaCoincidioEstaLinea && this.currentlyInSwitchBlock && mainFunctionBlockBraceDepth == this.switchBlockEntryDepth) {
                         if (this.currentSwitchClauseActiveAndNeedsDetener) { 
                            globalStructuralErrors.add("Error en bloque final (" + this.contentOfCurrentSwitchClauseStart + " en línea " + this.lineOfCurrentSwitchClauseStart + "): Se esperaba 'detener;' antes de cerrar el bloque 'sw'.");
                        }
                        this.currentlyInSwitchBlock = false; 
                ...
                if (lineaActual.equals("detener")) { 
                    ...
                    if (!balanceErrors.isEmpty()) {
                        ...
                        String[] commonStmtPatternsNoSemicolon = {
                            ...
                            "^Input\\s*.*?\\.Str\\(\\)$", "^Input\\s*.*?\\.Int\\(\\)$", "^Input\\s*.*?\\.Bool\\(\\)$"
                        };
        ...
        if (!mainFunctionDeclared) {
            ...
            globalStructuralErrors.add("Error ESTRUCTURAL GLOBAL: El bloque 'FUNC J2G Main()' no se cerró correctamente (faltan " + mainFunctionBlockBraceDepth + " llave(s) de cierre '}').");
        }
        
        if (this.currentlyInSwitchBlock) { 
            globalStructuralErrors.add("Error ESTRUCTURAL GLOBAL: El bloque 'sw' iniciado en línea " + this.switchActualOpeningLine + " no se cerró correctamente (posiblemente falta '}')."); 
            if (this.currentSwitchClauseActiveAndNeedsDetener) { 
        ...
        if (globalBraceBalance != 0 && !currentlyInMainFunctionBlock && mainFunctionDeclared) { 
            ...
            if (!mainOrSwitchBlockErrorAlreadyReported) { 
                 if (globalBraceBalance > 0) {
                    globalStructuralErrors.add("Error ESTRUCTURAL GLOBAL: Faltan " + globalBraceBalance + " llave(s) de cierre '}' en el programa.");
                 } else { 
                    globalStructuralErrors.add("Error ESTRUCTURAL GLOBAL: Hay " + (-globalBraceBalance) + " llave(s) de cierre '}' extra o mal colocadas en el programa.");
                 }
            }
        }
